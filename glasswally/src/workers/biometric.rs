...
fn structural_hash(prompt: &str) -> u64 {
    ...
    let first_word = prompt.split_whitespace()
        .next().unwrap_or("").to_lowercase();

    let dominant_verb = ["implement", "explain", "analyze", "generate", "translate",
                         "summarize", "compare", "evaluate", "describe", "list",
                         "write", "create", "provide", "show", "tell"]
        .iter()
...
pub async fn analyze(event: &ApiEvent, store: &StateStore) -> Option<DetectionSignal> {
    ...
    if n < 10 { return None; } // need sufficient history for reliable signal
    ...
    let norm_entropy = if max_entropy > 0.0 { (entropy / max_entropy) as f32 } else { 0.0 };
    ...
    let std  = (lengths.iter().map(|x| (x - mean).powi(2)).sum::<f64>()
    ...
    if prefix_ratio > 0.60 {
        score += 0.30;
        evidence.push(format!("template_sweep:{:.0}%_same_prefix", prefix_ratio * 100.0));
    } else if prefix_ratio > 0.40 {
        score += 0.15;
        evidence.push(format!("partial_template:{:.0}%_similar_prefix", prefix_ratio * 100.0));
    }
