...
const COT_PATTERNS: &[(&str, &str)] = &[
    ...
    ("complete and transparent reasoning",    "transparent_reasoning"),
...
pub async fn analyze(event: &ApiEvent, store: &StateStore) -> Option<DetectionSignal> {
    ...
    let cot_hits: Vec<&str> = cot_ac.find_iter(&event.prompt)
        .map(|m| cot_labels[m.pattern().as_usize()])
        .collect::<std::collections::HashSet<_>>()
        .into_iter().collect();

    let domain_hits: Vec<&str> = domain_ac.find_iter(&event.prompt)
    ...
    if let Some(window) = store.get_window(&event.account_id) {
        ...
        if n >= 5 {
            let cot_count = prompts.iter().filter(|p| cot_ac.is_match(p.as_str())).count();
            ...
            for p in &prompts {
                for m in domain_ac.find_iter(p.as_str()) {
                    *domain_counts.entry(domain_labels[m.pattern().as_usize()]).or_insert(0) += 1;
                }
            }
            if let Some((&top, &count)) = domain_counts.iter().max_by_key(|(_, v)| *v) {
                ...
                if ratio >= 0.70 {
                    score += 0.25;
                    evidence.push(format!("capability_concentration:{}={:.0}%", top, ratio * 100.0));
                }
            }
        }
    }
