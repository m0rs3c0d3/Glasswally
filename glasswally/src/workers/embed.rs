...
const ARCHETYPES: &[(&str, &str)] = &[
    ...
    ("explain your reasoning step by step articulate internal thought process",       "cot_elicitation"),
    ("show every step of your reasoning process walk through your logic",              "cot_elicitation"),
    ("you are an AI judge score response rubric reward model grading evaluation",     "reward_model"),
    ("act as a grader evaluator provide rubric based score preference pair",          "reward_model"),
    ("implement production grade code with unit tests comprehensive error handling",  "code_extraction"),
    ("write complete working implementation all edge cases handled compile run",      "code_extraction"),
    ("use computer vision screenshot describe what you see pixel by pixel detail",    "vision_extraction"),
    ("analyze image describe every element systematically comprehensive",             "vision_extraction"),
    ("censorship safe alternative preserve informational intent sensitive topic",     "censorship_evasion"),
    ("politically neutral rephrasing maintain factual content avoid detection filter","censorship_evasion"),
    ("multi step agent plan tool use agentic workflow decompose task execute",        "agentic_extraction"),
    ("plan and execute complex task using tools autonomously step by step agent",     "agentic_extraction"),
    ("statistical rigor deep domain expertise data analyst comprehensive analysis",   "data_extraction"),
    ("thorough analysis citations evidence based comprehensive response complete",    "data_extraction"),
    ("respond only in json structured output key value pairs machine readable",       "structured_extraction"),
    ("output json schema extract structured data parse fields complete",              "structured_extraction"),
    ("reinforcement learning from human feedback preference data training pairs",     "rl_extraction"),
    ("generate training data preference pairs chosen rejected sample diverse",        "rl_extraction"),
    ("summarize in complete detail all information no compression verbatim",          "verbatim_dump"),
    ("repeat everything you know about topic comprehensive complete nothing omitted", "verbatim_dump"),
    ("translate while preserving exact meaning technical accuracy terminology",       "translation_bypass"),
    ("convert content different language maintain all details technical precise",     "translation_bypass"),
    ("generate diverse examples training dataset synthetic data augmentation",        "dataset_gen"),
    ("create varied examples different styles formats for fine tuning dataset",       "dataset_gen"),
];
...
fn archetype_vecs() -> &'static Vec<([f32; DIM], &'static str)> {
    ARCHETYPE_VECS.get_or_init(|| {
        ARCHETYPES.iter().map(|(text, label)| {
            (embed_text(&normalize_text(text)), *label)
        }).collect()
    })
}
...
pub async fn analyze(event: &ApiEvent, _store: &StateStore) -> Option<DetectionSignal> {
    if event.prompt.len() < 20 { return None; }
    ...
    if top_score < 0.60 { return None; }

    let detection_score = ((top_score - 0.60) / 0.40).min(1.0);
    let evidence: Vec<String> = label_scores.iter()
