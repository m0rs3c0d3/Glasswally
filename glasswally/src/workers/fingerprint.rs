...
const REQUIRED_BROWSER_HEADERS: &[&str] = &["accept-language", "accept-encoding", "accept"];
...
pub async fn analyze(event: &ApiEvent, store: &StateStore) -> Option<DetectionSignal> {
        ...
        if is_script {
            ...
            if ua_claims_browser {
                ...
                evidence.push(format!(
                    "ua_tls_mismatch:ua=browser,ja3=script:{}",
                    &ja3[..8]
                ));
            } else {
                score += 0.15;
    ...
    if let Some(ref ja3s) = event.ja3s_hash {
        ...
        if accounts_with_ja3s.len() >= 10 {
            score += 0.12;
            evidence.push(format!(
                "ja3s_cluster:{}_accounts",
                accounts_with_ja3s.len()
            ));
        }
    ...
    if !event.header_order.is_empty() {
        let headers_lower: Vec<String> = event
            .header_order
            .iter()
        ...
        let script_hdrs: Vec<&str> = SCRIPT_INDICATOR_HEADERS
            .iter()
            .filter(|&&h| header_set.contains(h))
            .copied()
            .collect();
        ...
        if accounts_sharing.len() >= 10 {
            score += 0.20;
            evidence.push(format!(
                "header_cluster:{}_accounts_identical_order",
                accounts_sharing.len()
            ));
        }
    ...
    let n = store
        .get_window(&event.account_id)
        .map(|w| w.read().events.len())
        .unwrap_or(1);
...
fn header_entropy(observed: &[String]) -> f32 {
    let ref_pos: HashMap<&str, usize> = CHROME_ORDER
        .iter()
        .enumerate()
        .map(|(i, h)| (*h, i))
        .collect();
    let script_pos: HashMap<&str, usize> = PYTHON_REQUESTS_ORDER
        .iter()
        .enumerate()
        .map(|(i, h)| (*h, i))
        .collect();
