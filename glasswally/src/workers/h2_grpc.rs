...
const SCRIPT_LABELS:  &[&str] = &[
    "python_httpx", "python_aiohttp", "python_requests",
    "go_net_http2", "curl", "node_undici", "okhttp",
];
...
pub async fn analyze(event: &ApiEvent, _store: &StateStore) -> Option<DetectionSignal> {
    ...
    if let Some(label) = h2_label {
        ...
        if ua_claims_browser {
            score += 0.15;
            evidence.push(format!("h2_unknown_settings:fingerprint={}", h2.fingerprint));
        }
    }

    // gRPC detection (HTTP/2 + grpc content-type or user-agent)
    let is_grpc = event.header_order.iter()
    ...
    if h2.initial_window_size > 200_000_000 && h2_label.map(|l| SCRIPT_LABELS.contains(&l)).unwrap_or(false) {
        score += 0.15;
