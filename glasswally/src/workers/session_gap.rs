...
pub async fn analyze(event: &ApiEvent, store: &StateStore) -> Option<DetectionSignal> {
    ...
    let timestamps: Vec<i64> = {
        let w = window.read();
        let mut ts: Vec<i64> = w.events.iter()
    ...
    let gaps: Vec<f64> = sessions.windows(2)
    ...
    if gap_cv < 0.05 {
        score += 0.55;
        evidence.push(format!("cron_regularity:gap={:.0}s_cv={:.3}", mean_gap, gap_cv));
    } else if gap_cv < 0.08 {
        score += 0.40;
        evidence.push(format!("cron_regularity:gap={:.0}s_cv={:.3}", mean_gap, gap_cv));
    } else if gap_cv < 0.15 {
        score += 0.20;
        evidence.push(format!("semi_regular_gaps:gap={:.0}s_cv={:.3}", mean_gap, gap_cv));
    }
    ...
    let confidence = if gap_cv < 0.05 { 0.88 }
                     else if gap_cv < 0.08 { 0.75 }
                     else { 0.55 };

    let mut meta = HashMap::new();
    meta.insert("n_sessions".to_string(),
        serde_json::Value::Number(serde_json::Number::from(sessions.len() as u64)));
    meta.insert("mean_gap_secs".to_string(),
        serde_json::json!(mean_gap.round() as i64));
    meta.insert("gap_cv".to_string(),
        serde_json::json!((gap_cv * 1000.0).round() / 1000.0));
    meta.insert("session_size_cv".to_string(),
