...
pub async fn analyze(event: &ApiEvent, store: &StateStore) -> Option<DetectionSignal> {
    ...
    if rph > 200.0 {
        score += 0.45;
        evidence.push(format!("extreme_velocity:{:.0}rph", rph));
    } else if rph > 60.0 {
        score += 0.25;
        evidence.push(format!("high_velocity:{:.0}rph", rph));
    }
    ...
    if regularity > 0.70 {
        score += 0.30;
        evidence.push(format!("scripted_timing:{:.2}", regularity));
    } else if regularity > 0.50 {
        score += 0.15;
        evidence.push(format!("semi_regular:{:.2}", regularity));
    }
    ...
    if tokens.len() >= 5 {
        ...
        let std =
            (tokens.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / tokens.len() as f64).sqrt();
        let cv = if mean > 0.0 { std / mean } else { 0.0 };
        if cv < 0.15 {
            score += 0.15;
            evidence.push(format!("uniform_tokens:cv={:.3}", cv));
        }
    }

    // Off-hours (UTC 00:00–06:00 = CN business hours)
    let evs_24h = window.events_in(W_24HR);
    let off_count = evs_24h
        .iter()
        .filter(|e| {
            e.timestamp
                .format("%H")
    ...
    if off_ratio > 0.5 {
        score += 0.10;
        evidence.push(format!("off_hours_utc:{:.0}%", off_ratio * 100.0));
    }
